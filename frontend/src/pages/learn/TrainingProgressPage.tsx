...
import type { LearnerProgressSummary } from '@/types/learn'
import LoadingSpinner from '@/components/LoadingSpinner'

const DIMENSION_LABELS: Record<string, string> = {
  red_flag: 'Red flag identification',
  counseling_point: 'Counseling points',
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between items-center">
        <span className="text-xs text-gray-600">{DIMENSION_LABELS[label] ?? label}</span>
...
export default function TrainingProgressPage() {
  ...
  const hasWeaknesses = Object.keys(summary.weakness_breakdown).length > 0
  const topWeaknesses = Object.entries(summary.weakness_breakdown)
    .sort(([, a], [, b]) => b - a)
    .filter(([, rate]) => rate > 0)
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {hasWeaknesses && topWeaknesses.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">Weakness breakdown</h2>
          <p className="text-xs text-gray-400 mb-4">
            Proportion of your attempts where each safety dimension was missed.
          </p>
